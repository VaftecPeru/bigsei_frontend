...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchPaymentMethods = async () => {
      try {
        const response = await apiClient.get(
          Api_Global_Dashboard.director.porcentajeIngresos("")
        );
    ...
    const fetchPaymentMethodsE = async () => {
      try {
        const response = await apiClient.get(
          Api_Global_Dashboard.director.porcentajeEgresos("")
        );
  ...
  const getSede = async (text_search) => {
    try {
      const response = await apiClient.get(Api_Global_Dashboard.director.cantidadSedes(text_search));
      setSede({
  ...
  const getProfesores = async (text_search) => {
    try {
      const response = await apiClient.get(Api_Global_Dashboard.director.cantidadProfesores(text_search));
      setProfesores({
        cantidad: response.data.cantidad,
      });
    } catch (error) {
  ...
  const getEstudiantes = async (text_search) => {
    try {
      const response = await apiClient.get(Api_Global_Dashboard.director.cantidadEstudiantes(text_search));
      setEstudiantes({
        cantidad: response.data.cantidad,
      });
    } catch (error) {
  ...
  const getPadres = async (text_search) => {
    try {
      const response = await apiClient.get(Api_Global_Dashboard.director.cantidadPadres(text_search));
      setPadres({
        cantidad: response.data.cantidad,
      });
    } catch (error) {
  ...
  return (
    <div className="flex flex-col justify-start items-center min-h-screen text-2xl w-full bg-sky-50">
      ...
      <div className="flex flex-col lg:flex-row w-full max-w-7xl justify-center gap-4 p-4 rounded-2xl">
        ...
        <PercentageCard
          title="Porcentaje de ingreso por tipo"
          data={data}
        />
        <PercentageCard
          title="Porcentaje de egreso por tipo"
          data={dataE}
        />
      </div>
