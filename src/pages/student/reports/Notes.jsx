...
import GradesTable2 from '@/components/tables/GradesTable2';
...
function Notes() {
  ...
  const headers = ["Especialidad", "Ciclo", "Asignatura", "Evaluación", "Nota", "Formula"];
  ...
  const handleNotas = () => {
    setLoading(true);
    apiClient.get(Api_Global_Estudiante.reportes.notas(formulario.id_matricula))
      .then((response) => {
        setLoading(false);
  ...
  return (
    <div className="flex flex-col justify-start items-center min-h-screen text-lg w-full bg-sky-50 p-6">
      ...
      <div className="max-w-7xl w-full shadow-md rounded-lg bg-white">
        ...
        <div className="max-w-7xl w-full shadow-md rounded-lg bg-white">
          <GradesTable2 headers={headers} grades={notas} />
        </div>
      </div>
