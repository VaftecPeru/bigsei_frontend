    ...
    >
      ...
      <div className="w-full h-full">
        <img className="h-full absolute w-full top-0 right-0 h-full"
          src={perfil?.url_img} style={{ objectFit: "cover" }}></img>
      </div>
      ...
      >
        {archivoFoto ? (
          <img
            ...
            src={`data:image/png;base64,${ archivoFoto.url }`}></img>
        ) : (
...
export default function Perfil() {
  const { setIsClickledCard_docente, idPeriodocurso } = useContext(CursoContext);
  ...
  return (
    <div className="w-full rounded-2xl h-full bg-gray-100 overflow-hidden shadow-xl">
      <Perfil_first perfil={curso} onGoBack={clickFlechaLeft} />
      <Perfil_second perfil={curso} />
    </div>
  );
}
