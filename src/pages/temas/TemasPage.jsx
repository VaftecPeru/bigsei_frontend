...
import { MBACard2 } from "@/components/grados/MBACard2";
              ...
              <ul className="space-y-2">
                {categories.map((item) => (
                  <li
                    ...
                    className={clsx("flex items-center  hover:underline cursor-pointer  ",   categorySelected?.id_tipocategoria == item.id_tipocategoria ? "text-blue-600 underline font-semibold": "text-gray-600")}
                  >
            ...
            <div className="flex flex-wrap gap-4">
              {loading ? (
                ...
                courses.map((course, index) => (
                  <MBACard2
                    key={index}
                    title={course.curso_nombre}
                    slug={course.curso_nombre}
                    img={course.url_img}
                    description={course.detalle}
                    institution={course.empresa_razon_social}
                    path_url={`temas-detalle/${course.id_periodocurso}`}
                  />
                ))
              ) : (
