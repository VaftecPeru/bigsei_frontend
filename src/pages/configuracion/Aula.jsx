import { Pencil, PlusIcon, Search, Trash2, Home } from "lucide-react";
...
import AulaModal from "./modal/AulaModal";
...
export default function Aula() {
  ...
  const [idAulaEdit, setIdAulaEdit] = useState(null);
  const [idAulaDelete, setIdAulaDelete] = useState(null);
  const [searchQuery, setSearchQuery] = useState("");
  const columns = [
    { header: "N°", key: "index" },
    { header: "Aula", key: "nombre" },
  ...
  const handleConfirmDelete = () => {
    ...
    setIsLoading(true);
    apiClient.delete(Api_Global_Setup.aulas.eliminar(idAulaDelete))
      .then((response) => {
        setIsLoading(false);
  ...
  const handleEditClick = (row) => {
    setIdAulaEdit(row.id_aula);
    setIsOpenModal(true);
  };

  const handleAulas = (page_) => {
    setPage(page_);
    setIsLoading(true);
    apiClient.get(Api_Global_Setup.aulas.listar({
      per_page: perPage,
      page: page_,
    }, searchQuery))
      .then((response) => {
    ...
    <div className="flex flex-col justify-start items-center min-h-screen text-lg w-full bg-sky-50 p-6">
      <div className="w-full max-w-7xl bg-white">
        <div className="flex items-center gap-2 p-2 text-2xl">
          <Home size="80" className="text-gray-500" />
          <span class="font-bold text-gray-500">Aulas</span>
        </div>
        <hr />
      </div>
      <div className="w-full max-w-7xl p-2 bg-white">
        ...
        <TableSA5 columns={columns} rows={aulas} perPage={perPage} page={page} total={total} changePage={(data) => handleAulas(data)} />
      </div>
      ...
      {isOpenModal && <AulaModal onClose={() => {setIsOpenModal(false); setIdAulaEdit(null); handleAulas(page);}} idAulaEdit={idAulaEdit} />}
      <ToastContainer />
    </div>
  );
}