...
import CicloModal from "./modal/CicloModal";
...
export default function Ciclo() {
  ...
  const [idCicloEdit, setIdCicloEdit] = useState(null);
  const [idCicloDelete, setIdCicloDelete] = useState(null);
  const [searchQuery, setSearchQuery] = useState("");
  const columns = [
    { header: "N°", key: "index" },
    { header: "Ciclo", key: "nombre" },
  ...
  const handleConfirmDelete = () => {
    ...
    setIsLoading(true);
    apiClient.delete(Api_Global_Setup.ciclos.eliminar(idCicloDelete))
      .then((response) => {
        setIsLoading(false);
        toast.success("Realizado.");
        setIdCicloDelete(null);
        handleCiclos(page);
      })
      .catch((error) => {
        setIsLoading(false);
  ...
  const handleEditClick = (row) => {
    setIdCicloEdit(row.id_ciclo);
    setIsOpenModal(true);
  };

  const handleCiclos = (page_) => {
    setPage(page_);
    setIsLoading(true);
    ...
    <div className="flex flex-col justify-start items-center min-h-screen text-lg w-full bg-sky-50 p-6">
      <div className="w-full max-w-7xl bg-white">
        <div className="flex items-center gap-2 p-2 text-2xl">
          ...
          <span class="font-bold text-gray-500">Ciclos</span>
        </div>
        <hr />
      </div>
      <div className="w-full max-w-7xl p-2 bg-white">
        ...
        <TableSA5 columns={columns} rows={ciclos} perPage={perPage} page={page} total={total} changePage={(data) => handleCiclos(data)} />
      </div>
      ...
      {isOpenModal && <CicloModal onClose={() => {setIsOpenModal(false); setIdCicloEdit(null); handleCiclos(page);}} idCicloEdit={idCicloEdit} />}
      <ToastContainer />
    </div>
  );
}