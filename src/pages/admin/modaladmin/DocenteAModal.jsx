...
export default function DocenteAModal({ onClose, idDocenteEdit }) {
  const [isLoading, setIsLoading] = useState(false);
  const [photo, setPhoto] = useState(null);
  const [photoReal, setPhotoReal] = useState(null);
  const [tipoDocumentos, setTipoDocumentos] = useState([]);
  const [tipoNivelEducativos, setTipoNivelEducativos] = useState([]);
  const [tipoEspecializaciones, setTipoEspecializaciones] = useState([]);
  ...
  const handleRegistrar = (data) => {
    setIsLoading(true);
    apiFileClient.post(Api_Global_Admin.docentes.registrar(), data)
      .then((response) => {
        setIsLoading(false);
  ...
  const handleEditar = (data) => {
    setIsLoading(true);
    apiFileClient.post(Api_Global_Admin.docentes.editar(idDocenteEdit), data)
      .then((response) => {
        setIsLoading(false);
        toast.success("Realizado.");
        handleCloseModal();
  ...
  const handleNivelEducativos = () => {
    apiClient.get(Api_Global_Setup.tipoNiveleducativos.listar({
      per_page: 50,
      page: 1,
    }, ""))
      .then((response) => {
        setTipoNivelEducativos(response.data.data);
      })
      .catch((error) => {
        setTipoNivelEducativos([]);
        toast.warning(error.response.data);
      });
  };

  const handleEspecializaciones = () => {
    apiClient.get(Api_Global_Setup.tipoEspecializaciones.listar({
      per_page: 50,
      page: 1,
    }, ""))
      .then((response) => {
        setTipoEspecializaciones(response.data.data);
      })
      .catch((error) => {
        setTipoEspecializaciones([]);
        toast.warning(error.response.data);
      });
  };
  ...
  const handleDocente = () => {
    setIsLoading(true);
    apiClient.get(Api_Global_Admin.docentes.mostrar(idDocenteEdit))
      .then((response) => {
        setIsLoading(false);
        ...
        setValue("id_tipoespecializacion", response.data.id_tipoespecializacion);
        setValue("anhos_de_experiencia", response.data.anhos_de_experiencia);
        setValue("id_tiponiveleducativo", response.data.id_tiponiveleducativo);
  ...
  useEffect(() => {
    handleDocumentos();
    handleNivelEducativos();
    handleEspecializaciones();
    if (idDocenteEdit) {
      handleDocente(idDocenteEdit);
    }
  }, []);
  ...
  return (
    ...
    >
      <div className="w-full h-full max-w-4xl bg-white rounded-lg shadow-lg relative overflow-auto">
        ...
        <div className="p-6">
          <h1 className="text-2xl font-bold text-gray-900">Registrar Docente</h1>
        </div>
        <hr />
        ...
        <form className="space-y-6" onSubmit={handleSubmit(handleSave)} ref={formRef}>
          <div className="px-6 pt-6">
            ...
            <div className="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
              ...
              <div>
                <label htmlFor="id_tipoespecializacion" className="block text-sm font-medium text-gray-700">
                  Especialización:
                </label>
                <select
                  id="id_tipoespecializacion"
                  name="id_tipoespecializacion"
                  {...register("id_tipoespecializacion", { required: true })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option value="">--Seleccione--</option>
                  {tipoEspecializaciones.map((item) => (
                    <option value={item.id_tipoespecializacion} key={item.id_tipoespecializacion}>{item.nombre}</option>
                  ))}
                </select>
                {errors.id_tipoespecializacion && <span className="text-xs text-pink-500">Este campo es requerido</span>}
              </div>
              <div>
                <label htmlFor="anhos_de_experiencia" className="block text-sm font-medium text-gray-700">
                  Años de Experiencia:
                </label>
                <input
                  type="number"
                  id="anhos_de_experiencia"
                  name="anhos_de_experiencia"
                  {...register("anhos_de_experiencia", { required: true })}
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                />
                {errors.anhos_de_experiencia && <span className="text-xs text-pink-500">Este campo es requerido</span>}
              </div>
              <div>
                <label htmlFor="id_tiponiveleducativo" className="block text-sm font-medium text-gray-700">
                  Nivel Educativo:
                </label>
                <select
                  id="id_tiponiveleducativo"
                  name="id_tiponiveleducativo"
                  {...register("id_tiponiveleducativo", { required: true })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option value="">--Seleccione--</option>
                  {tipoNivelEducativos.map((item) => (
                    <option value={item.id_tiponiveleducativo} key={item.id_tiponiveleducativo}>{item.nombre}</option>
                  ))}
                </select>
                {errors.id_tiponiveleducativo && <span className="text-xs text-pink-500">Este campo es requerido</span>}
              </div>
            </div>
          </div>
