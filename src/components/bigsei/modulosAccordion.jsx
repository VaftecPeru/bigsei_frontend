...
import { Exam_icon, FlechaBajo_icon } from '@/pages/student/course/courseIcons';
import { Tema } from '@/pages/student/course/cursoContent';
    ...
    const semanas = [
        {
            ...
            actividades: [
                { tipo: "PRESENTACIÓN", descripcion: "Realizar estructura de control (PPT)", modal: "presentacion" },
                { tipo: "TAREA", descripcion: "Realizar estructura de control (PPT)", modal: "tarea" },
                { tipo: "CUESTIONARIO", descripcion: "responder las preguntas", modal: "cuestionario" },
                { tipo: "GRABACIÓN", descripcion: "Última clase 10/10/2024", modal: "verGrabacion" }
            ]
        },
        {
            id: 2,
            ...
            {isActivityOpen && semanas.map((semana) => (
                <React.Fragment key={semana.id}>
                    ...
                    <div className="w-[90%] bg-blue-300 h-[50px] transition duration-200 transition-all">
                        <div className="w-full bg-blue-50 h-full" onClick={() => handleWeekClick(semana.id)}>
